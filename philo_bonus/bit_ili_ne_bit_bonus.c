...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   bit_ili_ne_bit_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ladrian <ladrian@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/01 13:52:35 by ladrian           #+#    #+#             */
/*   Updated: 2022/06/11 13:01:53 by ladrian          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "bit_ili_ne_bit_bonus.h"
...
static void	what_a_wonderful_life(t_meta *meta, int id, sem_t *kushoet)
{
	...
	while (1)
	{
		logger(meta->common, id, THINK);
		sem_wait(meta->forks);
		logger(meta->common, id, FORK);
		sem_wait(meta->forks);
		logger(meta->common, id, FORK);
		sem_wait(kushoet);
		logger(meta->common, id, EAT);
		timemachine(meta->common->eattime, NULL);
		sem_post(meta->forks);
		sem_post(meta->forks);
		sem_post(kushoet);
		logger(meta->common, id, SLEEP);
